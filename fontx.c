...
// フォントファイルパスを構造体に保存
// Save font file path in structure
...
// UTFを含む文字列をSJISに変換
// Convert string containing UTF to SJIS
                ...
                uint8_t ssize)
{
  ...
  for (i = 0; i < stlen; i++)
  {
    ...
    // If the upper 4 bits are 1110, the 1st byte of the 3 byte character
      c1 = sp;
    }
    else if ((sp & 0xc0) == 0x80)
    { // 上位2ビットが10なら、他バイト文字の2バイト目以降
    // If the upper 2 bits are 10, after the second byte of the other byte character
      if (c2 == 0)
      {
        c2 = sp;
      }
