...
int main(int argc, char **argv)
{
      ...
      (strcmp(argv[1], "+f") == 0))
  {
    ...
    num = numc[1] - 'a';
    if (_DEBUG_)
      printf("add ASCII string to line [%d]\n", num);
      ...
      (strcmp(argv[1], "-6") == 0))
  {
    ...
    num = (numc[1] - '0') - 1;
    if (_DEBUG_)
      printf("delete string to line [%d]\n", num);
    sv.save[num].size = 0;
...
if(_DEBUG_)printf("set/unset reverse to line [%d]\n",num);
      ...
      (strcmp(argv[1], "P6") == 0))
  {
    ...
    if (col < 0 || col > 84)
      return 0;
    if (_DEBUG_)
      printf("set start colum to line [%d] = %d\n", num, col);
...
PYBIND11_MODULE(nokia, m)
{
  m.def("LCDInit", &LCDInit);
  m.def("LCDclear", &LCDclear);
  m.def("LCDdrawChar", &LCDdrawChar);
  m.def("LCDdisplay", &LCDdisplay);
}